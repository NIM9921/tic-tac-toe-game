...
def set_tile(row, column):
    ...
    if board[row][column]["text"] != "":
        ...
        tkinter.messagebox.showerror("Invalid Move", "This tile is already occupied!")
        return
...
for arow in range(3):
    for acolumn in range(3):
        board[arow][acolumn] = tkinter.Button(frame, text="", font=("Consolas",50,"bold"),
                                            background=color_gray, foreground=color_blue, width=4, height=1,
                                            command=lambda row=arow, column=acolumn: set_tile(row, column))
        board[arow][acolumn].grid(row=arow+1, column=acolumn)
